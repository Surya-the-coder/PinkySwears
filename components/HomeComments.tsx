...
const scrollToCard = () => {
    if (sessionStorage.currentPage === "HomeComments")
    {
...
const homecomments = (props:PropsWithChildren<any>) => {
    ...
    const [url,setUrl] = useState<any>(`/api/post/mostcommented/`)
    ...
    let postUrl = `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/post/mostcommented/`;
    let response
    let data
    ...
    let searchStringOnChange=(tempSearchString)=>{
        ...
        if(tempSearchString==""){
            ...
            url==='/api/post/mostcommented/'?null:setUrl(`/api/post/mostcommented/`)
        }
        else
        {
            ...
            <div className="flex justify-center bg-pink-200 min-h-screen bg-gradient-to-t from-[#FDEBF7] to-[#FFBCD1] w-full">
                ...
                <meta name='theme-color' content='#FFBCD1' />
                    ...
                    <div className={`flex justify-around mx-5 top-24 ${showSearch?'':null}`}>
                        ...
                        <button className={`bg-white text-[#FF848E] px-4 py-2 rounded-2xl focus:bg-[#F67A95] focus:text-white no-highlights text-sm ${showSearch?'hidden':null} `} onClick={loadRecentPage}>Recent</button>
                        ...
                        <button className={`bg-[#F67A95] text-white px-4 py-2 rounded-2xl no-highlights text-sm ${showSearch?'hidden':null} `} onClick={loadCommentsPage}>By comments</button>
                    </div>
